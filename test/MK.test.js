...
describe('Contracts', () => {
  ...
  const servicePct = 3
  const eventId = 1
  const title = 'SaintNFT title'
  const description = 'SaintNFT description'
  ...
  beforeEach(async () => {
    ...
    contract = await ethers.deployContract('MK', [servicePct])
  ...
  describe('Events Mgt.', () => {
    ...
    it('should confirm events update', async () => {
      ...
      const newTitle = 'second SaintNFT title'
      await contract
        .connect(owner)
        .updateEvent(
          eventId,
